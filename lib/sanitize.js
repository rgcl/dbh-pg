...
exports.object = function sanitizeObject(object, whitelist) {
    ...
    for(key in object) {
        if (object.hasOwnProperty(key) && whitelist[key]) {
            whiteValue = sanitizeScalar(key, whitelist[key], object[key]);
            if (whiteValue) {
...
exports.scalar = sanitizeScalar = function sanitizeScalar(value, whiteValue, metaValue) {
    if (!whiteValue) {
        return false;
    }
    ...
    if (whiteType === 'function') {
        return sanitizeScalar(value, whiteValue(value, metaValue), metaValue);
    }
    return value;
}
