...
module.exports = {
    ...
    exec : function(text, values) {
        ...
        if (!values) {
            // no values, so only call
            this.query.call(null, text, onReady);
        } else if (values instanceof Array) {
            // array values, so use the pg api
            this.query.call(null, text, values, onReady);
            ...
            this.query.call(null, text, arrayValues, onReady);
        }
    ...
    update : function(table, data, whereData, columnsData, columnsWhereData) {
        ...
        columnsWhereData = columnsWhereData || Object.keys(whereData);
        var values = [],
            valuesCount = 0,
            sql = 'UPDATE "'
                + table
